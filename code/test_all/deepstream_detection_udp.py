...
import socket
DETECTION_SEND_IP="127.0.0.1"
DETECTION_SEND_PORT=32770
...
def parse_face_from_meta(frame_meta, obj_meta):
    ...
    for i in range(num_joints):
        ...
        circle_params.circle_color.blue = 0.0
        circle_params.circle_color.alpha = 1.0
        circle_params.has_bg_color = 1
        circle_params.bg_color.red = 1.0
        circle_params.bg_color.green = 0.0
        circle_params.bg_color.blue = 0.0
        circle_params.bg_color.alpha = 1.0
        display_meta.num_circles += 1
        
def parse_obj_info(obj_meta):
    out_dict={}
    out_dict["xmin"]=obj_meta.rect_params.left*640/STREAMMUX_WIDTH
    out_dict["ymin"]=obj_meta.rect_params.top*512/STREAMMUX_HEIGHT
    out_dict["height"]=obj_meta.rect_params.height*512/STREAMMUX_HEIGHT
    out_dict["width"]=obj_meta.rect_params.width*640/STREAMMUX_WIDTH
    out_dict["confidence"]=obj_meta.confidence
    out_dict["class_id"]=obj_meta.class_id
...
def osd_sink_pad_buffer_probe(pad, info, user_data):
    ...
    while l_frame:
        ...
        result_this_frame={}
        result_this_frame["frame"]=frame_meta.frame_num
        result_this_frame["obj_count"]=frame_meta.num_obj_meta
        result_this_frame["source_frame_height"]=frame_meta.source_frame_height
        result_this_frame["source_frame_width"]=frame_meta.source_frame_width
        ...
        while l_obj:
            ...
            #info_this_obj={}
            #info_this_obj[]
            this_obj_dict=parse_obj_info(obj_meta)
            obj_list.append(this_obj_dict)
            ...
            try:
                ...
                break
        result_this_frame["object"]=obj_list
        #print(result_this_frame)
        out_string=json.dumps(result_this_frame)
        sock.sendto(out_string.encode(), (DETECTION_SEND_IP, DETECTION_SEND_PORT))
...
def main():
    ...
    sink.set_property('window-x', WINDOW_X)
    sink.set_property('window-y', WINDOW_Y)
    ...
    if not osd_sink_pad:
        sys.stderr.write('ERROR: Failed to get osd sink pad\n')
        sys.exit(1)
    else:
...
def parse_args():
    global SOURCE, CONFIG_INFER, STREAMMUX_BATCH_SIZE, STREAMMUX_WIDTH, STREAMMUX_HEIGHT, GPU_ID, \
        PERF_MEASUREMENT_INTERVAL_SEC,WINDOW_X,WINDOW_Y
    ...
    parser.add_argument('--window_x', type=int, default=0, help='window x min(default: 0)')
    parser.add_argument('--window_y', type=int, default=0, help='window y min(default: 0)')
